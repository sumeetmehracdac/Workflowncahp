...
const data = [
  { id: 1, action: "Accept", nextRoles: ["Member Secretary (PAC)", "Finance Officer"] },
  { id: 2, action: "Reject", nextRoles: ["Member Secretary (PAC)"] },
  { id: 3, action: "Send for Review", nextRoles: ["Committee Member (Expert Committee)", "Chairperson (Expert Committee)"] },
  { id: 4, action: "Forward to Committee", nextRoles: ["Committee Member (Expert Committee)"] },
  { id: 5, action: "Technically Recommend", nextRoles: ["Chairperson (Expert Committee)"] },
  { id: 6, action: "Send Sanction Letter", nextRoles: ["Director"] },
  { id: 7, action: "Return to Previous", nextRoles: ["Member Secretary (PAC)"] },
  { id: 8, action: "Hold", nextRoles: ["Director"] },
];

export default function NextRolePage() {
  return (
    <PageShell
      title="Next Role After Action"
      subtitle="Define workflow transition rules — after an action is performed, where does the proposal go next."
      layerBadge="Routing"
      layerColor="#0ea5a5"
      columns={[
      ...
      renderModal={(onClose) => (
        <FormModal title="Add Next Role Transition" onClose={onClose}>
          <FormField label="Action"><FormSelect options={["Accept", "Reject", "Send for Review", "Forward to Committee", "Technically Recommend", "Hold"]} placeholder="Select action" /></FormField>
          <FormField label="Next Role(s)">
            <div className="grid grid-cols-1 gap-2 mt-1">
              {["Member Secretary (PAC)", "Chairperson (Expert Committee)", "Committee Member (Expert Committee)", "Finance Officer", "Director"].map((r) => (
